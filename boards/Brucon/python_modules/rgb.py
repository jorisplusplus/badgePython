...
MAX_BRIGHTNESS = hub75.screenwidth
...
def scrolltext(text, color=(255, 255, 255), pos=None, width=hub75.screenwidth):
    if pos is None:
        pos = (0, int((hub75.screenheight - font_heights[current_font]) / 2))
...
def getbrightness():
    try:
        return nvs.get_int("system", "brightness")
...
def setbrightness(brightness=hub75.defaultbrightness):
    ...
    if brightness >= 1:
        nvs.set_int("system", "brightness", brightness)
