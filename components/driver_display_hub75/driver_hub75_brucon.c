...
#define BIT_LAT (1<<9) //connected to GPIO26 here
#define BIT_OE (1<<10)  //connected to GPIO25 here
...
esp_err_t driver_hub75_init(void)
{
	...
	for (int i=0; i<BITPLANE_CNT; i++) {
		for (int j=0; j<2; j++) {
			bitplane[j][i] = (uint16_t *) heap_caps_calloc(BITPLANE_SZ, sizeof(uint16_t), MALLOC_CAP_DMA);
			if (!bitplane[j][i]) {
