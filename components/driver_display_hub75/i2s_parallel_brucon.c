...
void i2sparallel_init(i2s_parallel_buffer_desc_t *bufa, i2s_parallel_buffer_desc_t *bufb) {
    ...
    hw.clkm_conf.val = 0;
    hw.clkm_conf.clkm_div_num = 2;
    hw.clkm_conf.clkm_div_b = 0;
    hw.clkm_conf.clkm_div_a =63;
    ...
    hw.conf.tx_msb_right = 1;
    hw.conf.tx_dma_equal = 1;

    hw.conf1.val = 0;
    hw.conf1.tx_pcm_bypass = 1;
    hw.conf1.tx_stop_en = 1;
    hw.timing.val = 0;
    //Set lcd mode
    hw.conf2.val = 0;
    hw.conf2.lcd_en = 1;
    ...
    hw.fifo_conf.tx_fifo_mod = 4;

    hw.conf_chan.tx_chan_mod = 0;//remove
    ...
    while (hw.conf.tx_reset==1 || hw.conf.tx_fifo_reset==1 || hw.conf.rx_fifo_reset==1) {}

    //Start dma on front buffer
    hw.lc_conf.val=I2S_OUT_DATA_BURST_EN | I2S_OUTDSCR_BURST_EN;
