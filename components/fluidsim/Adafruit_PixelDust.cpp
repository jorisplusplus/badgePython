...
void Adafruit_PixelDust::iterate(int16_t ax, int16_t ay, int16_t az) {
  ...
  ay -= az;                     // then...
  int16_t az2 = az * 5 + 1;     // max random motion to add back in
  ...
  for (i = 0; i < n_grains; i++) {
    ...
    dimension_t x_cor = grain[i].x / 256;
    dimension_t y_cor = grain[i].y / 256;

    if ((oldidx != newidx) && getPixel(newx / 256, newy / 256)) {  // If grain is moving to a new pixel but if pixel already occupied...
      delta = abs(newidx - oldidx);          // What direction when blocked?
      if (delta == 1) {                      // 1 pixel left or right)
        newx = grain[i].x;                   // Cancel X motion
        BOUNCE(grain[i].vx);                 // and bounce X velocity (Y is OK)
        if (y_cor < (height-1) && !getPixel(x_cor, y_cor+1)) grain[i].vy += 70;
        else if (y_cor > 0 && !getPixel(x_cor, y_cor-1)) grain[i].vy -= 70;
      } else if (delta == width) {           // 1 pixel up or down
        newy = grain[i].y;                   // Cancel Y motion
        BOUNCE(grain[i].vy);                 // and bounce Y velocity (X is OK)
        if (x_cor < (width-1) && !getPixel(x_cor+1, y_cor)) grain[i].vx += 70;
        else if (x_cor > 0 && !getPixel(x_cor-1, y_cor)) grain[i].vx -= 70;
